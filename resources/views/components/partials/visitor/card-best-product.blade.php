...
<div class="flex flex-col items-start space-y-2">
    <img src="{{ isset($favoritProduct->images[0]) ? Storage::url($favoritProduct->images[0]->name) : 'https://picsum.photos/300/300?random=' . rand(1, 1000) }}"
        width="300" height="300" alt="{{ $favoritProduct->name }}"
        class="mx-auto aspect-square overflow-hidden rounded-xl object-cover" />
    <div class="flex justify-between w-full">
        <div>
            <a href="{{ Route('detailProduct', [$favoritProduct->slug]) }}">
                <h3 class="text-lg font-medium">{{ $favoritProduct->name }}</h3>
            </a>
            <p class="text-sm text-gray-600">Rp{{ number_format($favoritProduct->price) }}</p>
        </div>
        <a href="https://wa.me/{{ $favoritProduct->url_wa }}?text=Halo,%20saya%20tertarik%20untuk%20memesan%20{{ urlencode($favoritProduct->name) }}.%20Bisakah%20anda%20memberikan%20informasi%20lebih%20lanjut?"
